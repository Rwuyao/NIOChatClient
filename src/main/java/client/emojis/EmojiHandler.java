...
public class EmojiHandler {
	private static final String path = "/view/emoji.json";
//	private static final String path = "./src/com/resources/emoji/emoji.json";
	...
	public Emoji getEmoji(String shortname) {
		Emoji entry = emojiMap.get(shortname);
	...
	public Map<String, List<Emoji>> getCategorizedEmojis() {
	...
	public Map<String, List<Emoji>> getCategorizedEmojis(int tone) {
		...
		getTonedEmojis(tone).forEach(emojiEntry -> {
			...
			//判断该表情包是否是我们需要的，不是则直接返回，是则加入map
			for (int i = 1; i <= 6; i++) {
				if (i == tone) continue;
	...
	public List<Emoji> getTonedEmojis(int tone) {
		//遍历所有的表情包，得到不同颜色的表情包，分别存储到变种list和默认list
		List<Emoji> allToned = new ArrayList<>();
		List<Emoji> selectedTone = new ArrayList<>();
		List<Emoji> defaultTone = new ArrayList<>();
		emojiMap.values().forEach(emojiEntry -> {
			for(int i = 1; i <= 5; i++) {
				if(emojiEntry.getShortname().endsWith("_tone" +i+":")) {
					...
					Emoji emojiEntryWithoutTone = emojiMap.get(withoutTone);
		...
		//如果要得是默认 就将默认加入，否则就是加入变种的表情包
		List<Emoji> allEmojis = new ArrayList<>(emojiMap.values());
